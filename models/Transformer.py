...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # (batch_size, n_heads, seq_len(=q_len), seq_len(=k_len))
...
class PositionWiseFeedForwardNetwork(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output = self.relu(self.linear1(inputs))  # (batch_size, seq_len, d_ff)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, inputs, attn_mask):
        ...
        attn_outputs, attn_weights = self.multi_head_attn(
...
class TransformerEncoder(nn.Module):
    def __init__(self,
        ...
        self.pad_id = pad_id
        self.sinusoid_table = self.get_sinusoid_table(
            seq_len + 1, d_model)  # (seq_len + 1, d_model)
    ...
    def forward(self, inputs):
        ...
        positions = torch.arange(
            inputs.size(1), device=inputs.device, dtype=inputs.dtype).repeat(
                inputs.size(0), 1) + 1
        position_pad_mask = inputs.eq(self.pad_id)
        positions.masked_fill_(position_pad_mask, 0)  # (batch_size, seq_len)
        ...
        attn_pad_mask = self.get_attention_padding_mask(
            inputs, inputs,
            self.pad_id)  # (batch_size, seq_len(=q_len), seq_len(=k_len))
    ...
    def get_attention_padding_mask(self, q, k, pad_id):
        attn_pad_mask = k.eq(pad_id).unsqueeze(1).repeat(
            1, q.size(1), 1)  # attn_pad_mask: (batch_size, q_len, k_len)
